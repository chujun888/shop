...
<body>
<h1>
<span class="action-span"><a href="/Back/Privilege/lst">权限列表</a></span>
<span class="action-span1"><a href="/Back/index/main">商之翼 管理中心</a> </span><span id="search_id" class="action-span1"> - 修改权限 </span>
...
<table width="100%" >
  <tr>
    ...
    <td><input type="text" name="pri_name" maxlength="60" value="<?php echo ($data["pri_name"]); ?>" /><span class="require-field">*</span></td>
  </tr>
  <tr>
      <input type='hidden' value='<?php echo ($data["id"]); ?>' name='id'/>
    <td class="label">上级权限:</td>
    <td><select name="parent_id"><option value="0">顶级权限</option><?php foreach($pris as $k=>$v):?><option value="<?php echo ($v["id"]); ?>" <?php if($v['id']==$data['parent_id']) echo 'selected';?>><?php echo str_repeat('&nbsp;',($v['level']+1)*5); echo ($v["pri_name"]); ?></option><?php endforeach;?></select><span class="require-field"></span></td>
  </tr>
  <tr>
      <input type='hidden' value='<?php echo ($data["id"]); ?>' name='id'/>
    <td class="label">模块名称:</td>
    <td><input type="text" name="module_name" maxlength="60" value="<?php echo ($data["module_name"]); ?>" /><span class="require-field">*</span></td>
  </tr>
  <tr>
      <input type='hidden' value='<?php echo ($data["id"]); ?>' name='id'/>
    <td class="label">控制器名称:</td>
    <td><input type="text" name="controller_name" maxlength="60" value="<?php echo ($data["controller_name"]); ?>" /><span class="require-field">*</span></td>
  </tr>
  <tr>
      <input type='hidden' value='<?php echo ($data["id"]); ?>' name='id'/>
    <td class="label">动作名称:</td>
    <td><input type="text" name="action_name" maxlength="60" value="<?php echo ($data["action_name"]); ?>" /><span class="require-field">*</span></td>
  </tr>
    <tr>
